...
from typing import Type, cast, Dict, Optional
...
class LightRAG:
    ...
    def query_with_keywords(
        ...
        """
        1. Extract keywords from the 'summary' using your new function in operate.py.
        2. Then run the standard aquery() flow with the final prompt (formatted_question).
        """

        loop = always_get_an_event_loop()
        return loop.run_until_complete(self.aquery_with_keywords(summary, param))

    async def aquery_with_keywords(
        self,
        summary: str,
        param: QueryParam = QueryParam()
    ):
        """
        1. Calls extract_keywords_only to get HL/LL keywords from 'summary'.
        2. Then calls kg_query(...) or naive_query(...), etc. with 'formatted_question' 
        as the main query, while also injecting the newly extracted keywords if needed.
        """
        ...
        mylogger.update_transaction(param.transaction_no,param.mode, **{
            "High-Level Keywords": hl_keywords, 
            "Low-Level Keywords": ll_keywords
            }
        )
        print("############################ Printing Keywords ####################")
        print("High-Level Keywords: ", hl_keywords)
        print("Low-Level Keywords: ", ll_keywords)
        # Combine them into a single string, or keep them separate. 
        # If your kg_query(...) function doesn’t yet accept keywords, you may want to store them in param:
        # combined_keywords = ", ".join(hl_keywords + ll_keywords)

        # (Optional) Place the extracted keywords somewhere so that kg_query can use them:
        # You might extend QueryParam with an extra field, e.g. param.extracted_keywords = combined_keywords
        param.hl_keywords=hl_keywords,
        param.ll_keywords=ll_keywords,
        # param.combined_keywords = combined_keywords
        ...
        print("####################### Printing Formatted Question #####################")
